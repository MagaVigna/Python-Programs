...
quantity=["one","two","three","four","five","six","seven","eight","nine"]
for menu in range(0,len(fruits_list)):
    print(menu+1," ",fruits_list[menu]) #printing the menu
...
for fruits in fruits_list: #checking if the input string has any fruit in it
    for item in new_order: 
        if item == fruits:
            ordered_fruit=item
            fruit_found=1
for count in quantity: #checking if the input string has quantity mentioned
    for item in new_order:
        if item == count:
